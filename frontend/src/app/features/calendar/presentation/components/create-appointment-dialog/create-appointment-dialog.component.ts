...
export class CreateAppointmentDialogComponent implements OnInit {
  ...
  selectedSlot = signal<CalendarSlotInterface | null>(null);
  availableSlots = signal<CalendarSlotInterface[]>([]);
  ...
  availableDentistsForDay = signal<DentistDto[]>([]);
    ...
    this.patientService.getAll(0, 10000, "person.lastName", "asc").subscribe({
      next: (
        ...
        if (response.data?.content) {
          // Convert PatientDto[] to PatientInterface[]
          this.allPatients = response.data.content.map((dto) =>
            this.patientSerializer.toView(dto),
          );
        }
      },
    });
  ...
  private loadSpecificPatient(patientId: number): void {
    this.patientService.getById(patientId).subscribe({
      next: (response: ApiResponseInterface<PatientInterface>) => {
        if (response.data) {
   ...
   * Selecciona un slot
   */
  selectSlot(slot: CalendarSlotInterface): void {
   ...
   */
  isSelectedSlot(slot: CalendarSlotInterface): boolean {
   ...
   */
  private loadDentistWeeks(dentists: DentistDto[]): void {
   ...
   */
  onDentistSelected(dentist: DentistDto): void {
   ...
   */
  isSelectedDentist(dentist: DentistDto): boolean {
